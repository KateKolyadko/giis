...
class MainWindow(QMainWindow):
    ...
    def show_3d_object_dialog(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if object_file:
                # Убрать префикс, если путь уже абсолютный
                object_file = object_file.strip()  # Удалить пробелы в начале/конце
                self.drawing_area = ViewerWithMenu()
                self.drawing_area.viewer.load_object(object_file)  # Использовать путь напрямую
